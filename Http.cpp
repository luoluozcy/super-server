...
//char CHttpProtocol::szIpAddr[256]=""; 

CHttpProtocol::CHttpProtocol(void)
{
	m_pListenThread = NULL;	
	//m_hwndDlg = NULL;
     m_strRootDir="E:";				// web的根目录
		m_nPort=8000;					// http server的端口号
...
{   //cout<<szIpAddr<<strlen(szIpAddr)<<endl;
	...
	{    
		// 错误处理   
		cout<<"Wrong WinSock Version"<<endl;
		return false;
	}
	...
	{
		// 异常处理
	//	CString *pStr = "Could not create listen socket";
		cout<<"Could not create listen socket"<<endl;
		return false;
	}
	...
	//	SendMessage(m_hwndDlg, LOG_MSG, (UINT)pStr, NULL);
	 ...
	 printf("Mini Httpserver已经启动 \n");
   printf("Mini Httpserver开始侦听 \n");
	// 创建listening进程，接受客户机连接要求
    m_pListenThread = AfxBeginThread(ListenThread, this);
// if (CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)ListenThread,NULL,0,m_pListenThread) == NULL)
   //{
   //  	cout<< "Couldn't start client thread"<<endl;
    	//delete pReq;
 // } 
	if (!m_pListenThread)
	{
...
//	PHOSTENT hostinfo;
	...
	if(gethostname(hostname, sizeof(hostname))==0)	// 得到主机名
	{
	 ...
	 sockaddr addr;
      int len=sizeof(addr);
	  getsockname(socketClient,&addr,&len);
	 // char szPort[10];
...
//	sprintf( szIpAddr,"%s,",szAddress );
	...
	*/
	...
	//	CString *pstr = new CString;
		...
		cout<<inet_ntoa(SockAddr.sin_addr)<<"Connecting on socket: "<<socketClient<<endl;
	...
	//	SendMessage(pHttpProtocol->m_hwndDlg, LOG_MSG, (UINT)pstr, NULL);
		...
		{   
			// 处理错误
		//	CString *pStr = new CString;
			cout<<"No memory for request"<<endl;
		//	SendMessage(pHttpProtocol->m_hwndDlg, LOG_MSG, (UINT)pStr, NULL);
			continue;
		}
	...
	{  
		// 超时返回，并且同步对象未退出
	//	CString *pStr = new CString;
		cout<< "One or more client threads did not exit"<<endl;
		//SendMessage(pHttpProtocol->m_hwndDlg, LOG_MSG, (UINT)pStr, NULL);
	}
...
{ // cout<<"hello3"<<endl;
	...
	{	
		// 处理错误
	//	CString *pStr = new CString;
		cout<<"Error occurs when analyzing client request"<<endl;
		//SendMessage(pHttpProtocol->m_hwndDlg, LOG_MSG, (UINT)pStr, NULL);
...
//	CString *pStr = new CString;
	cout<< &pReq->szFileName[strlen(m_strRootDir)]<<endl;
//	SendMessage(m_hwndDlg, LOG_MSG, UINT(pStr), NULL);	
	...
	{
		//CString *pStr = new CString;
	cout<< "Error occurs when closing file"<<endl;
	//	SendMessage(m_hwndDlg, LOG_MSG, (UINT)pStr, NULL);
	}
}
...
void CHttpProtocol::Disconnect(PREQUEST pReq)
{
	...
	int	nRet;
//	CString *pStr = new CString;
	cout<<"Closing socket: "<<pReq->Socket<<endl;
	//SendMessage(m_hwndDlg, LOG_MSG, (UINT)pStr, NULL);
	...
	//	CString *pStr1 = new CString;
		cout<<"closesocket() error: %d"<< WSAGetLastError() <<endl;
	//	SendMessage(m_hwndDlg, LOG_MSG, (UINT)pStr1, NULL);
	}
...
//	cout<<(UINT)&stats<<endl;
//	cout<<stats.dwRecv<<" "<<	stats.dwSend<<endl;
...
{  
	...
	GetLocalTime(&st);
	// 时间格式化
    wsprintf(lpszString, "%s %02d %s %d %02d:%02d:%02d GMT",week[st.wDayOfWeek], st.wDay,month
...
void CHttpProtocol::StopHttpSrv()
{
	...
	{
	//	CString *pStr = new CString;
	cout<<"TIMEOUT waiting for ListenThread"<<endl;
	//	SendMessage(m_hwndDlg, LOG_MSG, (UINT)pStr, NULL);
	}
	CloseHandle(m_hExit);
